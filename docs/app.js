...
// ===== Demo-Fallback =====
function makeBookFromMid(mid, feeBps=5, spreadBps=8){const fee=mid*(feeBps/10000);const spr=mid*(spreadBps/10000);return{bid:mid-fee-spr/2,ask:mid+fee+spr/2};}
...
function renderQuotes(symbol, quotes){
  const tb=TBody('#quotes'); tb.innerHTML='';
  for(const q of quotes){
    const tr=document.createElement('tr');
    tr.innerHTML=`
      <td>${q.exchange}</td><td>${symbol}</td>
...
function renderArbs(arbs){
  const tb=TBody('#arbs'); tb.innerHTML='';
  if(!arbs.length){ const tr=document.createElement('tr'); tr.innerHTML=`<td colspan="5" style="color:#a8b3c7">Keine positive Spanne</td>`; tb.appendChild(tr); return;}
  for(const a of arbs){
    const tr=document.createElement('tr');
    tr.innerHTML=`
      <td>${a.buyOn}</td><td>${a.sellOn}</td>
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('#refresh').addEventListener('change', ()=>{ if(timer){ clearInterval(timer); timer=null; start(); } });
  start();
});